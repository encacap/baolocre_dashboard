...
const Alert = ({
    ...
    const messageStack = message instanceof Error ? message.stack?.split("\n", 2)[1]?.trim() : "";
    ...
    return (
        <div className={twMerge(className, "flex p-4 rounded-xl", colorClassName[type])} {...otherProps}>
            <ToastIcon type="error" size={20} className="flex-shrink-0" />
            <div className="ml-4">
                ...
                {children && <div className="mt-2.5 text-sm font-semibold">{children}</div>}
            </div>
        </div>
    );
};
